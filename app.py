...
def load_css():
    ...
    .streamlit-expanderHeader {font-size: 0.9rem !important; font-weight: 600 !important; color: #ffffff !important; background: #e8e8f0 !important; border-radius: 8px !important; border: 1px solid #d0d0e0 !important;}
...
def main():
    ...
    with st.expander("📄 Fund Documents"):
        for fund, links in docs.items():
            st.markdown(f'<p style="color: #e8e8f0; font-weight: 700; font-size: 1.1rem; margin: 1.5rem 0 0.75rem 0;">{fund}</p>', unsafe_allow_html=True)
            cols = st.columns(3)
    ...
    # Chat Section
    st.markdown('<h4 style="margin: 1.5rem 0 1rem 0; color: #ffb8d0;">💬 Chat with FundWise AI</h4>', unsafe_allow_html=True)
